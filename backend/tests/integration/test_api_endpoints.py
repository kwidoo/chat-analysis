...
class TestFileEndpoints:
    def test_file_upload(self, client):
        ...
        # Create a test file
        data = {"file": (BytesIO(b'{"text": "This is a test document"}'), "test.json")}
...
class TestSearchEndpoints:
    def test_search(self, client):
        ...
        query_data = {"q": "test query", "k": 3}

        response = client.post(
            "/api/search", data=json.dumps(query_data), content_type="application/json"
        )
    ...
    def test_search_validation(self, client):
        ...
        # Invalid k parameter
        query_data = {"q": "test query", "k": "invalid"}
